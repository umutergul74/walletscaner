...
"discovery_gap_repair": "R21 uses migrations 044-046. A truncation captures its exact cursor before live admission advances, stages signature pages durably without moving that cursor, and replays oldest-first in bounded 50-item cycles. Completion must equal the immutable collected target, every staged row must be complete, a separate history-aware RPC query must prove that exact signature/slot successful and finalized, and post-incident WebSocket evidence must exist before coverage_reconciled_at can be set. The moving live cursor/latest head is never proof. Migration 046 preserves and normalizes the two pre-R21 mutable-cursor completion metadata rows only when their staged target and full replay are independently present. The unsafe R16 sessions remain failed; cap-exhausted intervals remain permanently alpha-excluded.",
